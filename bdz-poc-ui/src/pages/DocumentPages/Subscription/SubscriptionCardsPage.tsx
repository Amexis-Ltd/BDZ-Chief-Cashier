...
import { deleteCard } from '../../../store/slices/subscriptionCardsSlice';
...
const SubscriptionCardsPage: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const cards = useSelector((state: RootState) => state.subscriptionCards.cards);
  ...
  const handleOpenForm = (card?: SubscriptionCard) => {
    if (card) {
      navigate(`/documents/discount-cards/edit/${card.id}`);
    } else {
      navigate('/documents/discount-cards/add');
    }
  };
              ...
              <Table>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>Тип документ</TableCell>
                ...
                <TableBody>
                  {filteredCards.map((card) => (
                    <TableRow key={card.id}>
                      ...
                      <TableCell>
                        {documents.find(doc => doc.id === card.documentType)?.name || card.documentType}
                      </TableCell>
                      ...
                      <TableCell>
                        <IconButton
                          size="small"
                          onClick={() => navigate(`/documents/discount-cards/${card.id}/view`)}
                          title="Преглед"
                        >
                          <VisibilityIcon />
                        </IconButton>
